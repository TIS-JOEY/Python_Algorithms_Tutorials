...
因此，二元搜尋法的時間複雜度應為g\(N\) = 2 + log n \* 2

![](.gitbook/assets/nvs.logn.png)

可以看到上圖，當n的大小越來越大時，T\(n\)執行次數會遠大於T\(log n\)。因此很明顯的，二元搜尋法是優於循序搜尋法的演算法。
...
而我們會以θ\(最終主宰項\)來代表時間複雜度。

基於這個概念，以下列幾項較為常見的複雜度量級：  
θ\(1\) &lt; θ\(log n\) &lt; θ\(n\) &lt; θ\(N log n\) &lt; θ\(n ^ 2\) &lt; θ\(n ^ 3\) &lt; θ\(2 ^ n\) &lt; θ\(n!\)

上述可以看到量級間的大小關係，但當然這是當N夠大的前提，比如說當N&lt;10時，那麼θ\(2 ^ n\)都會小於θ\(n ^ 2\)，但當n&gt;=10以後，θ\(2 ^ n\)就必會大於θ\(n ^ 2\)。

#### Big O

接下來要介紹關於量級的嚴謹概念，讓我們可以更好地去對時間複雜度進行估計。其概念就是對於每一個g\(n\)，必存在某一個正整數常數c最終滿足g\(n\) &lt;= c \* f\(n\)。稱g\(n\)為f\(n\)的Big O，所代表的意義為g\(n\)最後最多只會與純最高次項所花費的時間相同而已，簡單來說，Big O就是在某函數上設置了漸進上限。

#### Big Ω

相對於Big O，Big Ω是在某函數上設置了漸進下限，其概念是對於每一個g\(n\)，必存在某一個正整數常數c最終滿足g\(n\) &gt;= c \* f\(n\)。

#### Big θ

Big θ代表的是給定一個給定的複雜度函數f\(n\)，存在θ\(f\(n\)\) = O\(f\(n\)\) ∩ Ω\(f\(n\)\)，也就是代表存在原始複雜度函數的成長率會漸進於某一個函數的成長率。 對於每一個g\(n\)，存在某一個正整數c和d，使c \* f\(n\) &lt;= g\(n\) &lt;= d \* f\(n\)。

#### Small O

相對於Big O是必存在某正實數c讓 g\(n\) &lt;= c \* f\(n\)，而small O則是所有的正實常數c通通都要成立。

### 定理 

#### 1. 若 g\(n\) ∈ o\(f\(n\)\)，則 g\(n\) ∈ O\(f\(n\)\) - Ω\(f\(n\)\)。

因g\(n\) ∈ O\(f\(n\)\) ，所以存在某正常數 c 使 g\(n\) &lt;= c\* f\(n\)。但因g\(n\) ∈ o\(f\(n\)\)，所以必要是所有正實常數c都要成立。但Ω\(f\(n\)\)卻要是存在某一正實常數c使 g\(n\) &gt;= c\* f\(n\)，因此產生矛盾，得證。

> 但o\(f\(n\)\)不一定都會與 O\(f\(n\)\) - Ω\(f\(n\)\)同一集合。

#### 2. g\(n\) ∈ O\(f\(n\)\) 若且唯若 f\(n\) ∈ Ω\(g\(n\)\)。

#### 3. g\(n\) ∈ θ \(f\(n\)\) 若且唯若 f\(n\) ∈ θ\(g\(n\)\)。

#### 4. 所有對數時間複雜度都在同一複雜度類別中。

#### 5. 所有指數時間複雜度並不在同一個時間複雜度類別中。
